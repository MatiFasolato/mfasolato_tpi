...
# --- 5. TEST DE MÁXIMOS Y MÍNIMOS (Producto más vendido) ---
def test_procesar_ventas_max_min():
    """Verifica que se identifique correctamente la mayor y menor compra por sucursal."""
    datos_prueba = [
        {COL_SUCURSAL: 1, COL_PRODUCTO: 'A', COL_CANTIDAD: 1, COL_PRECIO: 50},   # Total: 50 (Menor compra)
        {COL_SUCURSAL: 1, COL_PRODUCTO: 'B', COL_CANTIDAD: 2, COL_PRECIO: 100},  # Total: 200 (Mayor compra)
        {COL_SUCURSAL: 1, COL_PRODUCTO: 'C', COL_CANTIDAD: 1, COL_PRECIO: 150}   # Total: 150 (Medio)
    ]
    
    res = procesar_ventas(datos_prueba)
    sucursal_1 = res['sucursales'][0]
    
    # Validamos que encuentre los extremos correctamente
    assert sucursal_1['max_prod']['prod'] == 'B'
    assert sucursal_1['max_prod']['valor'] == 200.0
    assert sucursal_1['min_prod']['prod'] == 'A'
    assert sucursal_1['min_prod']['valor'] == 50.0
