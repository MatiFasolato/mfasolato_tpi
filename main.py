...
def ordenar_datos_burbuja(datos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Ordena únicamente por Sucursal (PRSUC).
    """
    lista = datos.copy()
    n = len(lista)
    for i in range(n):
...
# --- MAIN ---
def main():
    path_defecto = "data/supermercado_desordenado.csv"
    
    # Usamos try/except para detectar si estamos en GitHub Actions (donde no hay teclado)
    try:
        path = input(f"Path del CSV (Presione Enter para usar '{path_defecto}'): ").strip()
        if not path:
            path = path_defecto
            
        ordenar = input("¿Ordenar? (Y/N) [Default: Y]: ").upper().strip()
        if not ordenar:
            ordenar = 'Y'
    except EOFError:
        # Modo automático: se ejecuta si corre dentro del pipeline de GitHub
        path = path_defecto
        ordenar = 'Y'
